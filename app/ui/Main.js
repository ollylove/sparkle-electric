...
export default function Main({ children }) {
    ...
    // setting dark theme on button click in header
    const [darkTheme, setDarkTheme] = useState(false);
    function handleClick() {
        setDarkTheme(!darkTheme)
    }

    // setting dark theme when first visiting the website
    const firstVisit = true;
    useEffect(() => {
        if (firstVisit == true) setDarkTheme(true)
    }, [firstVisit])

    const pathname = usePathname()

    return (
        <div className={darkTheme ? 'main dark-theme' : 'main'}>
            <header className="d-flex justify-content-center align-items-center background-page">
                <button type="button" className="button secondary-bg" onClick={handleClick}>
                    <svg viewBox="0 0 455.881 455.881">
                        <g>
                ...
                <Image src={logo} alt="Sparkle Electric Logo" className="logo" />
                <Link href='/contacts' className={montserrat.className + ' button secondary-bg'}>Book a Call</Link>
            </header>
            <menu className={openSans.className + " d-flex"}>
                <Link href='/' className={pathname == '/' ? 'active' : ''}>Home</Link>
                <Link href='/about' className={pathname == '/about' ? 'active' : ''}>About</Link>
                <Link href='/residential' className={pathname == '/residential' ? 'active' : ''}>Residential Services</Link>
                <Link href='/commercial' className={pathname == '/commercial' ? 'active' : ''}>Commercial Services</Link>
            ...
            {/* all the content inside every page.js will be injected here */}
            {children}
